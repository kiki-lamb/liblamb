...
void lamb::device::Adafruit_ILI9341_STM_SPI2::spiwrite(uint16_t c) {
  ...
  if (hwSPI)
  {
    ...
    SPITWO.setClockDivider(11); // 8-ish MHz (full! speed!)
...
void lamb::device::Adafruit_ILI9341_STM_SPI2::begin(void) {
 ...
 #if defined (__AVR__)
     SPITWO.begin();
     SPITWO.setClockDivider(SPI_CLOCK_DIV2); // 8 MHz (full! speed!)
     SPITWO.setBitOrder(MSBFIRST);
     SPITWO.setDataMode(SPI_MODE0);
     mySPCR = SPCR;
 #elif defined(TEENSYDUINO)
...
void lamb::device::Adafruit_ILI9341_STM_SPI2::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1,
                                        uint16_t y1) {
...
*/
#if defined (__STM32F1__)
void lamb::device::Adafruit_ILI9341_STM_SPI2::drawLine(int16_t x0, int16_t y0,int16_t x1, int16_t y1, uint16_t color)
{
...
uint8_t lamb::device::Adafruit_ILI9341_STM_SPI2::readdata(void) {
  digitalWrite(_dc, HIGH);
  digitalWrite(_cs, LOW);
  uint8_t r = spiread();
  digitalWrite(_cs, HIGH);
...
uint8_t lamb::device::Adafruit_ILI9341_STM_SPI2::readcommand8(uint8_t c, uint8_t index) {
  if (hwSPI) spi_begin();
...
/*
 ...
 uint16_t lamb::device::Adafruit_ILI9341_STM_SPI2::readcommand16(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 ...
 uint32_t lamb::device::Adafruit_ILI9341_STM_SPI2::readcommand32(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 spiwrite(c);
